...
tabela1 <- dados %>%
  ...
  add_p(
    test = list(idade ~ "wilcox.test", all_continuous() ~ "t.test"),
...
fig_outcome <- ggplot(
  dados %>% filter(!is.na(act_semana12)), 
  aes(x = grupo, y = act_semana12, colour = grupo)) +
  # outlier.shape = NA para não duplicar os pontos do jitter
  geom_boxplot(outlier.shape = NA, width = 0.4, linewidth = 0.7) +
  geom_jitter(width = 0.12, alpha = 0.7, size = 2.5) +
  scale_colour_manual(
    values = c("App RespiRA" = "#2C3E50", "Controlo" = "#7A9B9E")
  ) +
  labs(
    x = "Grupo de Estudo",
    y = "Score ACT às 12 semanas",
    title = "RespiRA: Controlo da Asma às 12 semanas"
  ) +
...
tabela_final <- dados %>%
  select(grupo, act_semana12, delta_act, aqlq_semana12, adesao, exacerbacoes_12sem) %>%
  tbl_summary(
    by = grupo,
    ...
    statistic = list(
      all_of(c("act_semana12", "aqlq_semana12", "adesao")) ~ "{mean} ({sd})",
      all_of(c("exacerbacoes_12sem", "delta_act")) ~ "{median} ({p25}, {p75})"
    ),
    label = list(
      act_semana12 ~ "Score ACT Final",
      delta_act ~ "Variação do Score (Delta ACT)",
      aqlq_semana12 ~ "Score AQLQ (Qualidade de Vida)",
      adesao ~ "Adesão (%)",
      exacerbacoes_12sem ~ "Nº de Exacerbações"
    ),
    digits = all_continuous() ~ 1
  ) %>%
  add_p(
