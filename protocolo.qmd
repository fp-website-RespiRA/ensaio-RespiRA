...
# 6 Cronograma do Estudo (Tabela SPIRIT)
...
# 1. Definir os dados (Total de 6 colunas)
spirit_data <- data.frame(
  Atividade = c("Elegibilidade", "Consentimento", "Aleatorização", 
                "App RespiRA", "Cuidado Habitual", 
                "Score ACT", "Peak Flow", "AQLQ"),
  Recrutamento = c("X", "X", "", "", "", "", "", ""),
  Alocacao = c("", "", "X", "X", "X", "X", "X", "X"),
  S4 = c("", "", "", "X", "X", "X", "X", ""),
  S8 = c("", "", "", "X", "X", "X", "X", ""),
  S12 = c("", "", "", "X", "X", "X", "X", "X")
)

# 2. Gerar a tabela (A soma no header agora é 1+1+1+3 = 6)
spirit_data |>
  kbl(
    col.names = c("Atividade", "Recrutamento", "Alocação (T0)", "Semana 4", "Semana 8", "Semana 12"),
    align = "lccccc",
    caption = "Tabela SPIRIT: Cronograma do ensaio clínico RespiRA[cite: 1]"
  ) |>
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive")) |>
  # CORREÇÃO: A soma dos valores abaixo é 6 (1+1+1+3)
  add_header_above(c(" " = 1, "Inscrição" = 1, "Alocação" = 1, "Acompanhamento" = 3)) |>
  pack_rows("INSCRIÇÃO", 1, 3) |>
  pack_rows("INTERVENÇÕES", 4, 5) |>
  pack_rows("AVALIAÇÕES (OUTCOMES)", 6, 8)
```
